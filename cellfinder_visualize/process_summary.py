...
def plot_pooled_experiments(
    df_group_a, df_group_b, reference_structure_key, output_directory, boxplot=False,
):
    ...
    if len(df_group_a) > 1 and len(df_group_b) > 1:
        ...
        if boxplot:
            ...
            for metric, ax in zip(metrics_and_axis_labels.items(), axes_dict.values()):
                plt.sca(ax)
                plot_boxplots(df_group_a, df_group_b, metric=metric[0], label=metric[1])
        else:
...
def plot_boxplots(df_group_a, df_group_b, metric, label):
    ...
    for i, (name, group) in enumerate(df_group_a[['region', metric]].groupby('region')):
        values = group[metric].values
        names.append(name)
        all_values_a.append(values)

    bp = plt.boxplot(all_values_a, positions=range(0, len(all_values_a) * 2, 2))
    for item in ['boxes', 'whiskers', 'fliers', 'caps']:
        plt.setp(bp[item], color='k')

    for i, (name, group) in enumerate(df_group_b[['region', metric]].groupby('region')):
        values = group[metric].values
        all_values_b.append(values)

    bp = plt.boxplot(all_values_b, positions=np.arange(0, len(all_values_b) * 2, 2) + bar_space + padding, )
    for item in ['boxes', 'whiskers', 'fliers', 'caps']:
        plt.setp(bp[item], color='r')
    plt.ylabel(label)

    plt.xticks(np.arange(0, len(all_values_a) * 2, 2) + bar_space / 2, names,rotation=45)
...
def plot_cellfinder_bar_summary(
    ...
    plot_pooled_experiments(dfs_all[0], dfs_all[1], reference_structure_key, output_directory, boxplot=True)
    plt.pause(0.0001)
